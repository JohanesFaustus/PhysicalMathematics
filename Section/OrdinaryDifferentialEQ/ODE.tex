...
\end{equation*}
This will resul the change of argument in the Bessel equation from $Z(x)$ into $Z(Kx)$.

\subsubsection*{Recursion relation.} Here are few relation of Bessel function with its derivative.
\begin{flalign*}
    \begin{aligned}
        &\frac{d}{dx}[x^pJ_p(x)]=x^pJ_{p-1}(x)\\
        &\frac{d}{dx}[x^{-p}J_p(x)]=-x^{-p}_{p+1}(x)\\
        &J_{p-1}(x)+J(p+1)(x)=\frac{2p}{x}J_p(x)\\
