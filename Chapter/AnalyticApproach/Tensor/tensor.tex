...
Given two finite-dimensional vector spaces $V$ and $W$, we define their tensor product $V \otimes W$ to be the set of all $C$-valued bilinear functions on $V ^* \times W ^*$ with $\left\{ e_i \otimes f_j \right\}; i=1\dots,n,j=1,\dots ,m $ as basis such that
\begin{equation*}
