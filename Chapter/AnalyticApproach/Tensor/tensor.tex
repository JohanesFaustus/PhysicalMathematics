...
Operator in quantum mechanics are defined as $(1,1)$ tensor in Hermitian space $\mathcal{H}$.
The transformation is thus written as
\begin{equation*}
  H_{i'}{}^{j'}   = A^{k}_{i'} A^{l}_{j} H_{k}{}^{l}  
\end{equation*} 
with $H_{k}{}^{l}  $ as the component under defined basis 
\begin{equation*}
  H =  H_{k}{}^{l}  e ^{k } \otimes e _{l}
\end{equation*}
This of course nothing more that the similarity transformation.
The component can be written is Dirac notation as 
\begin{equation*}
  H_{k}{}^{l}  = \braket{k  | H|l}
\end{equation*}
The action of $H$ on state $\ket{\psi}$ projected in discrete basis $\left\{ \ket{k} \right\} $ is 
\begin{equation*}
  \bra{k} H \ket{\psi} = \sum_{l}H_{k}{}^{l} \psi_l =  \sum_{k,l} \ket{k }\bra{k }H \ket{l }\bra{l}
\end{equation*}
while in continuous basis $\left\{ \ket{x} \right\} $
\begin{equation*}
  (H\psi)(x) 
  =\int dx'\;\langle x| \hat{H} |x'\rangle \phi(x')
\end{equation*}
...
\subsection{Levi-Civita and Antisymmetric Tensor}
The Levi-Civita symbol (also called the permutation symbol) is a completely antisymmetric tensor defined in $n$ dimensions
\begin{equation*}
  \epsilon_{i_1,\dots,i_n} =
  \begin{cases}
    +1 & \text{if } (i_1,\dots,i_n) \text{ is an even permutation of } (1,\dots,n) \\
    -1 & \text{if } (i_1,\dots,i_n) \text{ is an odd permutation of } (1,\dots,n)  \\
    0  & \text{if any two indices are equal}
  \end{cases}
\end{equation*}
It is called antisymmetric since it satisfies
\begin{equation*}
    \epsilon_{i_{p(1)},\dots,i_{p(n)}} = \mathrm{sgn}(p)  \epsilon_{i_1,\dots,i_n} 
\end{equation*}
Since any permutation $p\in S_N$ of $N$ slots can be written as a product of transpositions
\begin{equation*}
...
We use the signature of permutation to denote this
\begin{equation*}
  \mathrm{sgn}(p) =
  \begin{cases}
...
\begin{equation*}
  S_N = \left\{ \sigma:\mathcal{N} \rightarrow \mathcal{N} \mid \sigma \text{ is bijective}  \right\} 
\end{equation*}
...
In three dimension, there are six permutation of $(123)$.
...
For comparison, a tensor is totally symmetric if
\begin{equation*}
  T_{i_1,\dots, i_n} = 
  T_{i_{p(1)},\dots, i_{p(n)}} 
\end{equation*}

\subsubsection{Levi-Civita contraction.}
Product of two Levi-Civita with third shared index can reduce into Kronecker delta
\begin{equation*}
  \epsilon_{ijk}\epsilon_{lmk} = \delta_{il } \delta_{jm} - \delta_{im } \delta_{jl}
\end{equation*}
We can also perform cyclic permutation to obtain the second shared index
\begin{equation*}
\epsilon_{jki}\epsilon_{mkl} = \delta_{jm} \delta_{il} - \delta_{jl} \delta_{im}
\end{equation*}
Once again, and we get the first shared index
\begin{equation*}
\epsilon_{kij}\epsilon_{klm} = \delta_{il} \delta_{jm} - \delta_{im} \delta_{jl}
\end{equation*}

The product is evaluated in the following step.
\begin{enumerate}
  \item The product is non-zero if $(i,j,k)$ are distinct and $(l,m,k)$  are also distinct. In three dimension, there are only three index $(1,2,3)$. For distinct $k$, that left with 2 index remaining. So it is impossible to all $(i,j,l,m)$ to be all distinct.  
  \item That the $(i,j)$ and $(l,m)$ to be the set of the same two indices.
  \item $(i,j,k)$ and $(l,m,k)$ gives a sign depending if they are odd or even permutation of $(1,2,3)$. If $i=l$ and $j=m$, the two sequences are the same ordering and thus the product gives the term
  \begin{equation*}
    (+1)(+1)=+1\qquad
    (-1)(-1)=+1
  \end{equation*}
  If $i=m$ and $j=l$, the two sequences are the swapped ordering and thus  the product gives the term
  \begin{equation*}
    (+1)(-1)=-1\qquad
    (-1)(+1)=-1
  \end{equation*}
\item This is enforced by the Kronecker delta
\begin{equation*}
  \epsilon_{ijk}\epsilon_{lmk} = \delta_{ij } \delta_{jm} - \delta_{im } \delta_{jl}
\end{equation*}
\end{enumerate}
\end{document}