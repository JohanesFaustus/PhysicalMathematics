...
\begin{align*}
    \mathrm{Re}\; z & =x                      &  & |z|=\text{mod } z=r=\sqrt{x^2+y^2}=\sqrt{zz^*} \\
    \mathrm{Im}\; z & =y\;\textrm{(not $iy$)} &  & \text{angle of } z=\theta
\end{align*}
The values of $\theta$ should be found from a diagram rather than a formula, although we do sometimes write $\theta = \arctan(y/x)$. Word of caution however, the domain of $\arctan x$ is restricted to $(-\pi/2, \pi/2)$.

Here's another useful (?) operator formula
\begin{align*}
...
The modulus of complex number is defined as
\begin{equation*}
    |z|=\sqrt{zz ^*}
\end{equation*}
so for $z=x+iy$, this applies
\begin{equation*}
    |z|^2 = x^2 + y^2 = \mathrm{Re} \;(z)^2 +\mathrm{Im} \;(z)^2
\end{equation*}

\subsection{Euler's Formula}
Using series expansion, we write $e^{i\theta}$, where $\theta$ is real
\begin{align*}
...
\subsection{Function of Complex Number}
We define $e^z$ by the power series
\begin{equation*}
...
With this, we can also express exponential with hyperbolic function
\begin{equation*}
