...
Regression, is employed when the data contain measurement noise or intrinsic variability, and the objective is to infer the underlying trend or functional relationship rather than to reproduce every observation.
...
Linear regression is a statistical method for modeling the relationship between two variables by fitting a straight line to data.
It assumes that the dependent variable $y$ depends linearly on an independent variable $x$ according to
\begin{equation*}
    y=mx+c
\end{equation*}
Each parameters can be determined with
\begin{equation*}
    m = \frac{n\sum xy - \sum x \sum y}{n\sum x^2 - (\sum x)^2},\qquad
    b = \bar{y} - m\bar{x}
\end{equation*}
Another important parameters is the Pearson coefficient
\begin{equation*}
    r = \frac{n\sum xy - \sum x \sum y}{\sqrt{(n\sum x^2 - (\sum x)^2)(n\sum y^2 - (\sum y)^2)}}, \\
\end{equation*}
which measures linear association between observed and predicted values, and RMSE
\begin{equation*}
    \text{RMSE} = \sqrt{\frac{1}{n}\sum_i (y_i - y_{p,i})^2}
\end{equation*}
which measures the typical magnitude of prediction error.
...
def linear(x,y):
    n=len(x)
\end{minted}
It first takes the set of data $(x,y)$ the takes the sample size.
\begin{minted}[breaklines]{python}
    sum_x=sum(x)
    x_bar=sum_x/n
    sum_y=sum(y)
...
\end{minted}
Then it calculate the required summation $\sum x_i$, $ \sum y_i$, $ \sum x_i^2$,$sum y_i^2$ and $\sum x_i y_i$.
\begin{minted}[breaklines]{python}
    m=(n*sum_xy-sum_x*sum_y)/(n*sum_x2-sum_x**2)
    b=y_bar-m*x_bar
    y_p=[m*xi+b for xi in x]
    r= (n * sum_xy - sum_x * sum_y)/ ((n * sum_x2 - sum_x**2) * (n * sum_y2 - sum_y**2)) ** 0.5
    rmse=(sum((y[i]-y_p[i])**2 for i in range (n))/n)**0.5
\end{minted}
Calculate the slope $m$, $y$-intercept, approximate value $y_p$, Pearson coefficient $r$, and root mean square error.
\begin{minted}[breaklines]{python}
    return [y_p,r,rmse,b]
\end{minted}
Then  we returns $y_p$, $r$, RMSE, and $b$. 
